...
For `"h265"`, the encoder tries multiple compatible codec strings (for example `hvc1` or `hev1`) to maximize browser support.
...
The buffer writer is limited to 2GB. Use `webFsWriter` or a custom writer for larger outputs.
